...
function CommentSidebar({ account, toggleCollapse }) {
  return (
    <div className="flex items-center flex-col w-[24px]">
      ...
      <div onClick={toggleCollapse} className="w-full h-full cursor-pointer bg-gradient-to-t from-canvas to-canvas bg-no-repeat bg-center hover:from-primary-500 hover:to-primary-500" style={{'background-size': '2px 100%'}} />
    </div>
  );
}
